...
# Always start from clean intermediates for release builds.
Write-Host "==> Cleaning previous build outputs..." -ForegroundColor Cyan
foreach ($d in @("publish", "KyeForge.App\bin", "KyeForge.App\obj", "KyeForge.Installer\bin", "KyeForge.Installer\obj")) {
    $p = Join-Path $root $d
    if (Test-Path $p) { Remove-Item $p -Recurse -Force }  # loud on purpose: a lock must fail the build, not go stale
}

# A running app locks its exe: publishing over it silently produces a stale build.
$pubExeCheck = Join-Path $root "publish\win-x64\KyeForge.exe"
$runningApp = Get-Process -Name "KyeForge" -ErrorAction SilentlyContinue | Where-Object { $_.Path -eq $pubExeCheck }
if ($runningApp) { throw "KyeForge is running from $pubExeCheck (PID $($runningApp.Id)). Close the app before building." }
...
$finalSetup = Join-Path $dist "KyeForge-Setup-1.0.4.exe"
